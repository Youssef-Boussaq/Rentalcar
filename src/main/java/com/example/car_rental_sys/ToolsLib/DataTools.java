...
public class DataTools {
    ...
    public static int getID(String tableName) {
        ...
        ArrayList<String[]> res = SQL.query(sql);
        if (res.size() > 0) {
            return (int) Float.parseFloat(res.get(0)[0]);
        }
        return 0;
    }
...
//        }
    ...
    public static boolean keepUserLoggedIn() {
        ArrayList<String[]> result = SQL.query("SELECT * FROM loginLog");
        for (int i = result.size() - 1; i >= 0; i--) {
            ...
            if (!Objects.equals(startTime, endTime)) {
                ...
                if (diff > 0) {
                    int userID = (int)Float.parseFloat(result.get(i)[1]);
    ...
    public static boolean logOut() {
        String sql = "UPDATE loginLog SET time2 = time1 WHERE time1 != time2";
        SQL.execute(sql);
        return true;
    }
